...
session_start();
  ...
  if(!isset($_SESSION["usuario"])){
    ...
    header("location:alumnonuevo.php");
  }
...
<div class="container-fluid">
  ...
  <form class="form-horizontal" action="registraralumno.php" method="post">
        ...
        <div class="form-group">
                                <label>Grado</label>
                <?php
                                     $sql="SELECT Nro_grado
                                     FROM grado
                                     order by Nro_grado";
                                     $rsC=mysqli_query($db,$sql);
                                     if ($rsC) {  
                                ?>
                                    <select  name="selgrado" />
                          <?php
                                              while ($r=mysqli_fetch_array($rsC)) 
                               { ?>
                             <option value=<?php echo $r["Nro_grado"]; ?>>
                           <?php echo $r["Nro_grado"]; ?>
                           </option>
                          <?php } ?>
                    </select>          
    ...
    <?php } 
      ...
      <form class="form-horizontal" action="registraralumno.php" method="post">
          ...
          <div class="form-group">
              <label for="direccion" class="col-lg-3 control-label" style="margin-top: 7px; text-align: center;">Dirección</label>
                  <div class="col-lg-8">
                    <input type="direccion" REQUIRED class="form-control" name="direccion" placeholder="Dirección">
                  </div>
          </div>
          <div class="form-group">
                  ...
                  <?php
                                     ...
                                     if ($rsC) {  
                                ?>
                                    <select  name="selgrado" style="height: 40px; width: 50px; ">
                          <?php
                                              while ($r=mysqli_fetch_array($rsC)) 
                               { ?>
                             <option value=<?php echo $r[Nro_grado]; ?> style="text-align: center;">
                           <?php echo $r["Nro_grado"]; ?>
                           </option>
                          <?php } ?>
                    </select>          
